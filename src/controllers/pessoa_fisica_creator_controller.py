...
class PessoaFisicaCreatorController(CreatorControllerInterface):
  ...
  def __validate_renda_mensal(self, renda_mensal: float) -> None:
    if not isinstance(renda_mensal, (int, float)):
      raise HttpBadRequestError('Renda mensal deve ser um número.')
    
  def __validate_idade(self, idade: int) -> None:
    if not isinstance(idade, (int, float)):
      raise HttpBadRequestError('Idade deve ser um número.')
